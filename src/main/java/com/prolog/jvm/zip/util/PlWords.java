 ...
 * realization in the Warren Abstract Machine (WAM) found in [1]. The WAM's take
 * on words differs in several respects from that found in the literature on the
 * ZIP (see [2]), but both adopt the basic premise of their decomposition into a
 * tag and a value, making the two notions largely interchangeable. Besides some
 * differences in the naming conventions for tags, the deviations from [2] are
 * outlined in the list below. In summary, the WAM's specification for words is
 * mostly a simpler one, motivating our preference towards it.
 * <ul>
...
public final class PlWords {
    ...
    /**
     * Tag used for functors. The value of a functor consists of a pointer to to
     * a data aggregate containing both the atom and arity.
     */
    ...
    /**
     * Returns the value of the supplied {@code word}, consisting of its 28
     * lower-order bits.
     */
