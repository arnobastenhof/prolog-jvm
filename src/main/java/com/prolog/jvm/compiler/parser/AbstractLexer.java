...
public abstract class AbstractLexer implements Lexer {
    ...
    private char lookahead = ' ';   // lookahead character, initialized with
                                    // whitespace
     ...
     *
     * @param input a reader for a Prolog source program or query; not allowed
     * to be null
    ...
    /**
     * Convenience method serving as an alternative to {@link #consume()} in
     * case it is known that the lookahead character is not a linefeed.
     *
     * @throws IOException
     */
     ...
     * @throws IOException
     * @throws RecognitionException if {@code expected} does not match
     * {@link #getLookahead()}
     */
    protected final void match(final char expected) throws IOException,
            RecognitionException {
        if (this.lookahead == expected) {
            consumeNonLinefeed();
        } else {
            throw RecognitionException.newInstance(this.lookahead, this.line,
