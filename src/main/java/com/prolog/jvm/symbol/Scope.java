...
/**
 * Class representing a possibly nested scope for storing Prolog
 * {@link Symbol}s.
 * <p>
 * The design of this class is based loosely on Pattern 17 of Parr (2010),
 * Language Implementation Patterns: Symbol Table for Nested Scopes. The primary
 * differences lies in the logic for resolving symbols: whereas Parr recursively
 * searches the parent scope until a symbol is found, the current class instead
 * delegates this responsibility to the client code, searching only the current
 * scope.
 * <p>
 * With {@link Symbol}s identified as such only through a marker interface, this
 * class allows for the typesafe retrieval of specific implementation classes by
 * * parameterizing the type of the key whereby they are stored, thus adopting
 * the typesafe heterogeneous container pattern described in Item 29 from Bloch
...
public final class Scope {
     ...
     * search the parent scope, leaving this instead as a responsibility for the
     * client code.
     *
     * @param key the search key; not allowed to be null
     * @throws NullPointerException if {@code key == null}
